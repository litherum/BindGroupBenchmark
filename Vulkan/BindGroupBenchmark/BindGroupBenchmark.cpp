...
void Demo::prepare_descriptor_set() {
	...
	auto const image_create_info = vk::ImageCreateInfo()
		...
		.setExtent({ (uint32_t)4, (uint32_t)4, 1 })
		.setMipLevels(1)
		.setArrayLayers(1)
		.setSamples(vk::SampleCountFlagBits::e1)
		.setTiling(vk::ImageTiling::eOptimal)
	...
	std::vector<vk::Image> images(maximum);
	std::vector<vk::DeviceMemory> memoryAllocations(maximum);
	std::vector<vk::ImageView> imageViews(maximum);

	vk::Format const tex_format = vk::Format::eR8G8B8A8Unorm;
	for (int i = 0; i < maximum; ++i) {
		auto result = device.createImage(&image_create_info, nullptr, &images[i]);
		VERIFY(result == vk::Result::eSuccess);

		vk::MemoryRequirements mem_reqs;
		device.getImageMemoryRequirements(images[i], &mem_reqs);

		auto memoryAllocationInfo = vk::MemoryAllocateInfo();
		memoryAllocationInfo.setAllocationSize(mem_reqs.size);
		memoryAllocationInfo.setMemoryTypeIndex(0);

		auto pass = memory_type_from_properties(mem_reqs.memoryTypeBits, vk::MemoryPropertyFlagBits::eDeviceLocal, &memoryAllocationInfo.memoryTypeIndex);
		VERIFY(pass == true);

		result = device.allocateMemory(&memoryAllocationInfo, nullptr, &(memoryAllocations[i]));
		VERIFY(result == vk::Result::eSuccess);

		result = device.bindImageMemory(images[i], memoryAllocations[i], 0);
		VERIFY(result == vk::Result::eSuccess);
		...
		result = device.createImageView(&viewInfo, nullptr, &imageViews[i]);
		VERIFY(result == vk::Result::eSuccess);
	}
	...
	for (int item = 1; item < maximum / step; ++item) {
		...
		vk::DescriptorSetLayoutBinding const layout_binding = vk::DescriptorSetLayoutBinding()
																	   .setBinding(0)
																	   .setDescriptorType(vk::DescriptorType::eSampledImage)
																	   .setDescriptorCount(i)
																	   .setStageFlags(vk::ShaderStageFlagBits::eFragment)
																	   .setPImmutableSamplers(nullptr);

		auto const descriptor_layout = vk::DescriptorSetLayoutCreateInfo().setBindingCount(1).setPBindings(&layout_binding);

		vk::DescriptorSetLayout descriptorSetLayout;
		auto result = device.createDescriptorSetLayout(&descriptor_layout, nullptr, &descriptorSetLayout);
		VERIFY(result == vk::Result::eSuccess);

		for (int underscore = 0; underscore < trials; ++underscore) {
			...
			for (uint32_t j = 0; j < i; j++) {
				textureDescriptors[j].setImageView(imageViews[j]);
				textureDescriptors[j].setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal);
			}
			vk::WriteDescriptorSet write;
			write.setDstBinding(0);
			write.setDescriptorCount(i);
			write.setDescriptorType(vk::DescriptorType::eSampledImage);
			write.setPImageInfo(textureDescriptors.data());
