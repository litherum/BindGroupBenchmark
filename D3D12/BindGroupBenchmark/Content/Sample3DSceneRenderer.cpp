...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	auto createAssetsTask = createPipelineStateTask.then([this]() {
		...
		// Create a descriptor heap for the constant buffers.
		{
			...
			heapDesc.NumDescriptors = DX::c_frameCount + maximum;
		...
		CD3DX12_RESOURCE_DESC textureDescription = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R8G8B8A8_UNORM, 4, 4);
		std::vector<Microsoft::WRL::ComPtr<ID3D12Resource>> textures(maximum);
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE baseHandle = cbvCpuHandle;
		for (int item = 1; item < maximum / step; ++item) {
			...
			for (int underscore = 0; underscore < trials; ++underscore) {
				...
				for (int j = 0; j < i; ++j) {
					...
					d3dDevice->CreateShaderResourceView(textures[item].Get(), &desc, cbvCpuHandle);
				}
